...
def takeAttendance(request):
    if request.method == 'POST':
        ...
        if Attendence.objects.filter(date = str(date.today()),branch = details['branch'], year = details['year'], division = details['division'],period = details['period']).count() != 0 :
            ...
            students = Student.objects.filter(branch = details['branch'], year = details['year'], division = details['division'])
            ...
            for student in students:
                if str(student.registration_id) in names:
                    ...
                    division = details['division'])
                    attendence.save()
            attendences = Attendence.objects.filter(date = str(date.today()),branch = details['branch'], year = details['year'], division = details['division'],period = details['period'])
